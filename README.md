...
每个助理还可维护最多 50 个一次、每日或每周自动任务，支持暂停、恢复、编辑、删除和立即执行。任务可显式选择最多 5 个助理文件，并在已授权范围内读取指定页面或保存整页截图；未选择的文件和浏览器能力不会被隐式继承。任务结果会进入对应助理时间线，并通过站内角标、浏览器或 Electron 系统通知提醒用户。助理配置、对话、文件引用、任务、工具摘要和执行历史均由 NearChat 持久化，服务重启或运行时热重载后仍可继续；Mastra 不接管用户会话数据，也没有任意写文件权限。
