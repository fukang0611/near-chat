...
| 个人助理   | 多助理、多条独立对话、模型与知识库绑定、文件工作区、自动任务、受控浏览器与完整执行记录                 |
...
侧栏“智能助理”入口提供独立的私人工作台。每个用户最多创建 20 个助理，可从通用、写作、分析和规划预设开始，自定义角色说明与头像颜色，并组合最多 10 个自己的知识库。同一助理最多保留 30 条可命名对话，每条对话分别保存历史、草稿、活动时间和自动任务；归档只隐藏并暂停所属任务，助理角色、模型、知识库与文件工作区继续共享。旧版单时间线会自动迁移为“默认对话”。每个助理可单独授权跨会话消息检索和私人记忆读取，默认均关闭；Mastra 只获得服务端注入的当前用户与允许会话范围，实际使用过的来源会随回复持久化，点击即可定位原消息或目标记忆。每个助理还拥有最多 30 个文件的独立工作区，可复用聊天文件或单独上传；用户在某轮提问中明确勾选后，服务端才提取受支持文档的文字交给模型，未选择的文件不会被隐式读取。助理回复可由用户显式保存为 Markdown 或 TXT，并继续通过原有 MinIO 权限和配额体系下载、引用与回收。

聊天输入器支持从结构化 `@` 菜单选择自己的智能助理。请求会先生成仅发起者可见的私人预览，用户确认后才作为带 AI 身份标识的正式消息发送到当前会话；也可以把预览放回输入框继续编辑或直接丢弃。聊天内助理只读取当前会话的公开上下文，服务端强制移除跨会话检索、私人记忆和私人助理说明等能力，避免把个人信息带入群聊回复。AI 未配置、被管理员关闭或暂时不可用时，普通文字、图片、文件和其他现有聊天能力不受影响。

每个助理还可维护最多 50 个一次、每日或每周自动任务，支持暂停、恢复、编辑、删除和立即执行。任务可显式选择最多 5 个助理文件，并在已授权范围内读取指定页面或保存整页截图；未选择的文件和浏览器能力不会被隐式继承。独立的“日程”工作区以列表或月历统一展示任务的下一次执行和最多 100 条个人提醒，支持完成、推迟、重新安排与删除；到期事件通过站内提示、浏览器或 Electron 系统通知送达，点击通知可回到对应助理、对话和日程。助理配置、对话、文件引用、提醒、任务、工具摘要和执行历史均由 NearChat 持久化，服务重启或运行时热重载后仍可继续；Mastra 不接管用户会话数据，也没有任意写文件权限。

受控浏览器按助理默认关闭。手动创建的执行不会立即访问网络，打开、读取、截图、点击或填写均需逐步确认；自动任务仅可预授权无副作用的页面读取或截图，不能点击和填写。服务端只接受页面快照生成的元素编号，不开放任意 CSS、脚本、密码框、文件上传或下载；会话使用独立无痕上下文，完成、取消或空闲超时后立即销毁。目标、脱敏后的页面快照、步骤状态、自动执行标记与截图文件均保留在本人助理工作区，任务历史可以直达对应记录，便于追踪和复核。
...
- 个人助理保留当前对话的最近上下文生成回复，完整时间线、草稿与自动任务按线程隔离，并支持重命名、归档、恢复和清空。
...
- 自动任务仅能读取逐项选择的文件，并可预授权页面读取或截图；点击和填写始终保留人工逐步确认。
...
npm run smoke:assistant-context # 跨会话/记忆工具、来源持久化、权限撤销
npm run smoke:assistant-mention # 结构化 Mention、私有预览、确认后的公开助理回复
```
